...
def animate(i):
    ...
    # ax[1].bar(df["Datetime"], df["MACD_Hist"].where(df["MACD_Hist"] >= 0), color="green", alpha=0.8, label="MACD Positive")
    # ax[1].bar(df["Datetime"], df["MACD_Hist"].where(df["MACD_Hist"] < 0), color="red", alpha=0.8, label="MACD Negative")
